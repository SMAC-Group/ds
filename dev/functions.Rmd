...
```{r, eval=FALSE}
my_lm <- function (response = NA, covariates = NA) {
...
where `response` and `covariates` become the names of the variables that will be used within the body of the function. You can notice that we assigned the value `NA` to both of these variables. This is their "default" value which, if not specified otherwise, is used as input for the function (which in this case would either give an error or output `NA`).
