...
In Table \@ref(tab:tableprtexample) and the plot below we can observe that Netflix has high risk and high return. Apple seems like a safer investment in terms of risk, but lower in expected return. Our portfolio optimization investment, however, seems to be in a good area, in that it has low risk and a return that is between the two stocks mentioned above.  

```{r, echo = FALSE, fig.cap="Plot of Expected Return and Variance / Risk"}
plot(investment_summary[2, ], investment_summary[1, ], ylab = "Expected Value / Return", xlab = "Variance / Risk", pch = 19, cex = 1, lty = "solid", lwd = 2, xlim = c(min(investment_summary[2, ]), max(investment_summary[2, ]+1e-04)), col = c(4,2,3))
text(investment_summary[2, ], investment_summary[1, ], labels = names(investment_summary[1, ]), cex= 0.7, pos = 4)
```

Below, we can analyze such risk and return situations from other companies such as those demonstrated below.  

![](images/finance_ex.png)

```{block2, type='rmdnote'}
Note that we may revisit this example in the next chapter where we will discuss a more flexible solution to this problem.
```
