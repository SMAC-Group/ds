...
Try to apply algorithm to the maze presented above. Could you escape? Though it might take some time and probably is not the fastest strategy one can show that this method can solve any maze (assuming of course that a solution exists). In this section we discuss how to actually program the "`repeat (until "you are free")`" part of our algorithm. 
