...
The reason for defining this statistic as *Excess* Kurtosis lies in the fact that the standardized kurtosis is compared to that of a Gaussian distribution (whose kurtosis is equal to 3) which has exponentially decaying tails. Consequently, if the Excess Kurtosis is positive, this implies that the distribution has heavier tails than a Gaussian and therefore has higher probabilities of extreme events occurring. To understand why the Excess Kurtosis is equal to 0 for a Gaussian distribution click on button below:

<button id="hidebutton">Excess Kurtosis Derivation</button>
<div id="hideclass">
...
Matrices are another extremely common data structure in R. Compared to vectors, matrices have an additional dimension which, for example, allows to stock multiple equidimensional (same length) vectors within the same object. Below is an example of how to create a matrix containing consecutive numbers in R:
...
If you interested in understanding how Equation \@ref(eq:omega) was obtain click on the button below:

<button id="hidebutton">Minimum Variance Portfolio Derivation</button>
<div id="hideclass">
...
Using \@ref(eq:omega), we obtain that the expected value and variance of our investement are given by

\[
\begin{aligned}
\text{Expected Value Investement} &=\left[\omega^* \mu_A + (1 - \omega^*) \mu_N\right] W,\\
\text{Variance Investement} &=  \left[(\omega^*)^2 \sigma_A^2 + (1 - \omega^*)^2 \sigma_N^2 + 2 \omega^* (1 - \omega^*) \sigma_{AN}\right]W^2.
\end{aligned}
\]

In practice, the vector $\boldsymbol{\mu}$ and matrix $\boldsymbol{\Sigma}$ are unknown and must be estimated on historical data. In the code below, we compute the optimal value of $\omega$ based on \@ref(eq:omega) using the last five years of historical data for the two stocks considered.
...
# Compute investement expected value and variance
mu_investement = omega_star*mu[1] + (1 - omega_star)*mu[2]
var_investement = omega_star^2*Sigma[1,1] + (1 - omega_star)^2*Sigma[2,2] + 
  2*omega_star*(1 - omega_star)*Sigma[1,2]
```

From this code, we obtain $\omega^* \approx$ `r round(100*omega_star, 2)`%. In the table below we compare the empirical expected values and variances of the stocks stocks as well as those of our investement:

```{r tableprtexample}
investement_summary = matrix(NA, 2, 3)
dimnames(investement_summary)[[1]] = c("Expected value", "Variance")
dimnames(investement_summary)[[2]] = c("Apple", "Netflix", "Investement")
investement_summary[1, ] = c(mu, mu_investement)
investement_summary[2, ] = c(diag(Sigma), var_investement)
knitr::kable(investement_summary, caption = "TO DO")
```

In Table \@ref(tab:tableprtexample) we can observe that our ...


```{r}
plot(sqrt(c(diag(Sigma), var_investement)), c(mu, mu_investement))
```
