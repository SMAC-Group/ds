...
Table: (\#tab:tennis) Five best male single tennis players (as ranked by ATP on 07-15-2017).
...
It can be noticed that this data set contains columns with a variety of data types and in the next sections we will use these data to illustrate how to work with five types of data structrures mentionned earlier.
...
`c()` is a generic function which combines arguments to form a vector. All arguments are coerced to a common type which is the type of the returned value, and all attributes except names are removed. For example, let us consider the number of grand slams won by the five players considered in the eighth column of Table \@ref(tab:tennis): 
