...
### C++ in a R package
Writing a R package with C++ code follows essentially the same steps
as a regular R package. Here we notify the main differences:
...
3. Move your R scripts into the R folder. Here in addition, **move the C++ files into the `src` folder**; 
...
4. Documentation: follow the exact same step for R files. For C++ files, documentation
uses the same syntax except that `#` should be replaced by `//` (comments in C++).  
5. The next steps are identical.   
