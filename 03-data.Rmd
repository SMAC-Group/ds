...
This is a little surprising as all the elements in `grand_slam_win` are integers and it would seem natural to expect this as an output of the function `typeof()`. This is because R considers any number as a "double" by default, except when adding the suffix `L` after an integer. For example:
...
When working with dates it is useful to treat them as real dates than character strings that *look* like dates (to a human) but don't otherwise *behave* like dates. For example, consider a vector of three dates: `c("03-21-2015", "12-13-2011", "06-27-2008")`. 

The `sort()` function returns the elements of a vector in ascending order, but since these dates are actually just character strings that *look* like dates (to a human), R sorts them in alphanumeric order (for characters) than chronological order (for dates):
...
Suppose that one is interested in analyzing the behavior of Apple's stock price over the last three months. The first thing needed to perform such analysis is to recover (automatically) today's date.  In R, this can be obtained as follows
...
Matrices are another extremely common data structure in R. Compared to vectors, matrices have an additional dimension which, for example, allows to stock multiple equi-dimensional (same length) vectors within the same object. Below is an example of how to create a matrix containing consecutive numbers in R:
...
It is often the case however that we already have equi-dimensional vectors available and we wish to stock them into a matrix. In these cases, two useful functions are `cbind()` - column bind - and `rbind()` - row bind - where the first function stocks the vectors vertically side-by-side while the second stocks the vectors horizontally one below the other. An example of the former is given below:
...
In practice, the vector $\boldsymbol{\mu}$ and matrix $\boldsymbol{\Sigma}$ are unknown and must be estimated on historical data. In the code below, we compute the optimal value of $\omega$ based on \@ref(eq:omega) using the last five years of historical data for the two stocks considered. For simplicity we set $W$ to one but note that $W$ plays no role in determining $\omega^*$
...
In Table \@ref(tab:tableprtexample) and the plot below we can observe that Netflix has high risk and high return. Apple seems like a safer investment in terms of risk, but lower in expected return. Our portfolio optimization investment, however, seems to be in a good area, in that it has low risk and a return that is between the two stocks mentioned above. Finally, we provide a graphical representation of the obtained results in the figure below. 

```{r, echo = FALSE, fig.height = 5, fig.width = 6, fig.align = "center", warning = FALSE, results = 'hide', fig.cap = "Graphical repsresentation of Apple, Netflix and min-variance portfolio based on these two assets."}
plot(investment_summary[2, ], investment_summary[1, ], ylab = "Daily Expected Value of Investement", xlab = "Daily Investement Variance", pch = 19, cex = 1, lty = "solid", lwd = 2, xlim = c(min(investment_summary[2, ]), max(investment_summary[2, ]+1e-04)), col = c(4,2,3))
grid()
text(investment_summary[2, ], investment_summary[1, ], labels = names(investment_summary[1, ]), cex= 0.7, pos = 4)
```

In the next chapter we will revisit this example where we will new programming tools to consider a more flexible solution to this investment problem.
