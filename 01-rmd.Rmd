...
Citations and bibliographies can automatically be generated with RMarkdown. In order to use this feature we first need to create a "BibTex" database.  is a plain text file. The file must be given the extension bib. Here are some ways to create one.

file where all our references will be store. 

we are able to use BibTex to generate bibliographies.
BibTex is a way to structure output of paper citations you can obtain from
Google Scholar

For example, it a recent paper two researchers from Oxford University investigated the connection between the taste of food and various aspects cutlery such as weight and colour. The called this phenomon the "taste of cutlery". The bibtex "entry" for this paper is given below:
...
i,e. how plateware can affect the perceived taste and flavour of food, but very little is known about visual and proprioceptive influences of cutlery on the response of consumers to the food sampled from it
