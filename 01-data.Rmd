...
Naturally, the difference between the two in general is relatively unimportant but we can see that `grand_slam_win_int` takes less "space" to two. For example, we have
...
As indicated earlier, a vector has a homogenous data structure meaning that it can only contain a single type among all the data types. Therefore, when more than one data type is provided, R *coerces* the data into a "shared" type. To identify this "shared" we can use this simple rule:
