...
RMarkdown is a framework that provides a literate programming format for data science. It can be used to save and execute R code within RStudio and is also a simple formatting syntax for authoring HTML, PDF, ODT, RTF, and MS Word documents. More specifically, an RMarkdown is a plain text file that contains three different building blocks:

- YAML metadata
- Code Chunks 
- Text 

We will discuss the features in the following sections.

What is **literate** programming? 

Literate programming is the notion of adding narrative context along with the code to simultaneously produce documententation for the code or program being developed. Consequently it is possible to read through the code with explanations so that any viewer can follow through the lines of code. RMarkdown offers a simple tool that allows to create reports or presentation slides in a reproducible manner with collateral advantages such as avoiding repetitive tasks by, for example, changing all figures or re-executing computations when data are updated. 

What is **reproducible** research?

Reproducible research or reproducible analysis is the notion that an experiment's whole process, including collecting data, performing analysis and producing outputs can be reproduced the same way by someone else. Building non-reproducible experiments has been a problem both in research and in the industry, and having such an issue highly decreases the credibility of authors' findings and eventually, the authors themselves. In essence, allowing for reproducible research implies that someone will have the possibility of running code and obtaining the same exact results as the original research and RMarkdown is commonly used to address this issue. 
...
We will later enforce this notion by utilizing `set.seed()` option in the following chapters. 


## YAML Metadata 

YAML stands for *YAML Ain't Markup Language*, and is used to specify document configurations and properties such as name, date, output format, etc. The (optional) YAML header is delimited by "---" above and below.

![](images/rmd_yaml.png)


You can also include additional formatting [options](http://rmarkdown.rstudio.com/html_document_format.html), such as a table of contents or even custom CSS which can be used to further enhance presentation. For the purpose of this book, the default options should be sufficient. Below is an example knit output of the above RMarkdown file. 
...
The default output above is an html_document format. However, the format of the output can also be specified with options such as `pdf_document` (see previous link). Nevertheless, before you proceed to knitting (compiling) your Rmarkdown document, you must install a TeX distribution such as [MikTeX](https://miktex.org/2.9/setup) with which you can also include raw LaTeX files and even define LaTeX macros (more about LaTeX further on in this chapter).
...
The main feature of RMarkdown documents is the possibility of embedding R code within its outputs. We therefore start by showing how R code can be embedded in RMarkdown documents through so-called "chunks" that can quickly be inserted in the document using the following options:
...
Once the chunk has been inserted, it is possible to type in the R code you are interested in. The following paragraphs discuss different options to manage your chunks of R code. There is also the possibility of giving a name to the chunk (say for example `name`) by adding this name directly after the `r` at the beginning of the chunk as follows: ````{r name}`.

However, there are a series of options that can be used to customize and execute the embedded R code. If additional options are specified, a comma (",") must be placed after the `r` (and eventual name of the chunk) following which additional arguments can be specified to define the different options. The latter are discussed in the following paragraphs.

### Code Chunk Options

Some examples of chunk options can be seen in the initial example of this chapter. For instance, in the third chunk we can notice that the `echo = FALSE` parameter was added to the code chunk to prevent printing the R code that generated the plot. This is a useful way to embed figures. The main options can be found in the [Rmarkdown Cheatsheet](https://www.rstudio.com/wp-content/uploads/2015/02/rmarkdown-cheatsheet.pdf) and Yihui's notes on knitr [options](https://yihui.name/knitr/options/) while here were simply list some explanations of the most commonly used chunk options that are taken from these references: 
...
- warning: (TRUE; logical) whether to preserve warnings (produced by warning()) in the output that is shown in the R console (if FALSE, all warnings will be printed in the console instead of the output document);
- cache: (FALSE; logical) whether to cache a code chunk (more on this option later on in this chapter).

There are also a series of options that can be used when the chunk includes plotting functions. Here are a few: 
...
The variables we store in an RMarkdown document will be part of the environment of the document itself. This means that we can call and manipulate them anywhere within the document. In addition, we can also reference the variable "in-line" within the text section using the \`r something\` syntax. So suppose that we assign a value to an object as follows:

```{r}
a = 2
```

In the above case we stored the value 2 in an object we called `a`. To refer to this value within the text we can use the following syntax for example

```{r, eval = FALSE, highlight = FALSE}
The value of $a$ is `r a`. 
```

which translates in R Markdown to "The value of $a$ is `r a`."

## Text 

Due to its literate nature, text will be an essential part in explaining your analysis as can be seen through the highlighted sections of our initial RMarkdown example reproduced below.

![](images/rmd_text.png)

Firstly, with RMarkdown we can specify custom text formatting with emphases such as *italics*, **bold**, or even a `code style`. Indeed, the latter sentence would be typed out as follows in RMarkdown:

```{r, eval = FALSE, highlight = FALSE}
[...] such as with emphasis such as *italics*, **bold**, or even a `code style`.
```

In the following subsections, we will discuss other options and features to develop the text in RMarkdown documents.

### Headers {-}

As can be seen in the above example, headers are always preceded by the "#" symbol where an increased number of successive "#"s delivers smaller headings and therefore allows for nested subsections. 
...
If you do not wish to add captions to the picture, then simply follow the syntax below leaving the caption square brackets blank:
