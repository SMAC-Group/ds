...
What are the advantages of these different inferential methods? Which approach appears to be preferable to the others and under what circumstances? We can run some specific simulation studies to get an idea of the answers to these questions but, to do so, we will need to create functions the implement these methods. The next sections will present the main features of functions in R, starting from the components and calling of a function and ending with special calls and return values.

## Function components

R functions have three main components to them:
...
We will briefly go over these components by building a function implementing the non-parametric bootstrap to find a confidence interval for the sample mean which we saw in the previous chapter:
...
- "B" which represents the number of bootstrap sample we wish to generate;
