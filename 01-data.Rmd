...
Therefore let us use the abovementioned dataset to estimate Hubble's constant to then get an estimate of the age of the universe. This dataset can be found in the `gamair` package under the name `hubble` and, if plotting the two variables of interest in a scatterplot there appears to be a positive linear relationship between the two variables.
...
age.year = age.sec/(60^2*24*365) # Age of the universe in years
```

Based on this estimation, the age of the universe appears to be almost 13 billion years. However, we know that $\hat{\beta}$ is a random variable which therefore follows a distribution which, based on asymptotic statistical theory, is a normal distribution with expectaton $\beta$ and variance $\sigma^2(X^TX)^{-1}$. Suppose that we have a hypothesis on the age of the universe, for example that of 

```{r}
# Estimate of the residual variance from Eq. (1.3)
resid = hubble$y - hubble$x*beta.hat
sigma2.hat = (1/df)*t(resid)%*%resid

# Estimate of the variance of the estimated beta from Eq. (1.2)
var.beta = sigma2.hat*solve(t(hubble$x)%*%hubble$x)

# Linear regression with lm() function
fit_lm = lm(hubble$y~hubble$x-1)

# Compare outputs
manual_results = c(beta.hat, sigma2.hat)
function_results = c(fit_lm$coefficients[1], (1/df)*t(fit_lm$residuals)%*%fit_lm$residuals)
results = cbind(manual_results, function_results)
row.names(results) = c("beta.hat", "sigma2.hat")
results
```

I think it is quite cool as it can be used to estimate the "age" of the universe and test wether the estimate of the age of the universe by Creation Scientists (based on a reading of the Bible) is reasonable. This is an example based on Simon Woods book. If we do this example, we should add something on confidence interval and I think he could the normal distribution (instead of t-distribution) to avoid going into details. Anyway the t-test relies on Normal assumption which is hard to verify. In the following chapter, we could use this example to show the various form of boostrap (parametric, non-parametric, semi-parametric). Let me know what you think.
...
# Plot data
plot(y = hubble$y, x = hubble$x, col="red", main="Cepheid Distance vs. Galaxy Velocity", ylab = "Galaxy Relative Velocity (in km/s)", xlab = "Distance (in Megaparsecs)")
...
# Estimate of beta from Eq. (1.1)
beta.hat = solve(t(hubble$x)%*%hubble$x)%*%t(hubble$x)%*%hubble$y
