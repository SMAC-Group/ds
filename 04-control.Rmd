...
Common errors: `2 = 2` others????
```

```{block2, type='rmdnote'}
**Difference between || | and & &&**

`c(TRUE,FALSE) || c(FALSE, FALSE)` or `c(TRUE,FALSE) && c(FALSE, FALSE)`.
...
- `x & y`: if `x` is `FALSE` then `y` is not evaluated. Therefore, if `y` contains a some kind of error using such expression can create bugs that are hard to find. For example, consider the expression `y = x && 2 == NULL`, then if `x` is `FALSE` `y` will be `FALSE` while if `x` is `TRUE` `y` will be `NA`!
- Similarly, `x | y` if `x` is `TRUE` then `y` is not evaluated.
```
